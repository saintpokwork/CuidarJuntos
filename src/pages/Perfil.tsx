  ...
  return (
    <DashboardLayout>
      <main className="flex-1 w-full relative ">
        <DashboardPageHeader title={t('pages.profile.caretaker')} showSearch={false} />
          ...
          <div className="glass-card rounded-[24px] p-8 soft-shadow border border-white/40 mb-8">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-body-md text-on-surface-variant">{t('pages.profile.caretaker')}</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.name')} *</label>
                <input
                  disabled={!editMode}
                  value={form.nome}
              ...
              <div className="space-y-3">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.birthDate')}</label>
                <input
                  disabled={!editMode}
                  value={form.dataNascimento}
                  onChange={(e) => setForm((prev) => ({ ...prev, dataNascimento: e.target.value }))}
                  className="w-full h-12 px-4 bg-surface border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary/20 outline-none disabled:cursor-not-allowed"
                  placeholder={t('pages.profile.birthDatePlaceholder')}
                />
              </div>
              <div className="space-y-3 md:col-span-2">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.address')}</label>
                <input
                  disabled={!editMode}
                  value={form.morada}
              ...
              <div className="space-y-3">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.snsNumber')}</label>
                <input
                  disabled={!editMode}
                  value={form.numeroSNS}
                  onChange={(e) => setForm((prev) => ({ ...prev, numeroSNS: e.target.value }))}
                  className="w-full h-12 px-4 bg-surface border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary/20 outline-none disabled:cursor-not-allowed"
                  placeholder={t('pages.profile.snsPlaceholder')}
                />
              </div>
              <div className="space-y-3">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.familyDoctor')}</label>
                <input
                  disabled={!editMode}
                  value={form.medicoFamilia}
                  onChange={(e) => setForm((prev) => ({ ...prev, medicoFamilia: e.target.value }))}
                  className="w-full h-12 px-4 bg-surface border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary/20 outline-none disabled:cursor-not-allowed"
                  placeholder={t('pages.profile.familyDoctor')}
                />
              </div>
              <div className="space-y-3">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.pharmacy')}</label>
                <input
                  disabled={!editMode}
                  value={form.farmaciaHabitual}
                  onChange={(e) => setForm((prev) => ({ ...prev, farmaciaHabitual: e.target.value }))}
                  className="w-full h-12 px-4 bg-surface border border-outline-variant rounded-xl focus:ring-2 focus:ring-primary/20 outline-none disabled:cursor-not-allowed"
                  placeholder={t('pages.profile.pharmacy')}
                />
              </div>
              <div className="space-y-3 md:col-span-2">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.allergies')}</label>
                <input
                  disabled={!editMode}
                  value={form.alergias}
              ...
              <div className="space-y-3 md:col-span-2">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.conditions')}</label>
                <input
                  disabled={!editMode}
              ...
              <div className="space-y-3 md:col-span-2">
                <label className="text-label-sm font-bold text-on-surface block">{t('pages.profile.importantNotes')}</label>
                <textarea
                  disabled={!editMode}
                  ...
                  rows={4}
                  placeholder={t('pages.profile.importantNotes')}
                />
              </div>
            </form>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="glass-card rounded-[24px] p-6 soft-shadow border border-white/40">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-headline-md font-headline-md text-on-surface">{t('pages.profile.allergies')}</h3>
              </div>
            ...
            <div className="glass-card rounded-[24px] p-6 soft-shadow border border-white/40">
              <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-headline-md font-headline-md text-on-surface">{t('pages.profile.conditions')}</h3>
              </div>
          ...
          <div className="glass-card rounded-[24px] p-6 soft-shadow border border-white/40 mt-6">
            <div className="flex items-center gap-3 mb-4">
              <span className="material-symbols-outlined text-primary text-2xl">contacts</span>
                <h3 className="text-headline-md font-headline-md text-on-surface">{t('pages.profile.primaryContacts')}</h3>
            </div>
